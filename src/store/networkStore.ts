  ...
  persist(
    (set, get) => ({
      ...
      lockDevice: (id) => {
        ...
        get().updateClient(id, { locked: true });
      },

      unlockDevice: (id) => {
        window.lhgSystem?.sendNetworkCommand({ deviceId: id, command: { type: 'unlock' } });
        get().updateClient(id, { locked: false });
      },

      shutdownDevice: (id) => get().shutdownClient(id),
      restartDevice: (id) => get().restartClient(id),
      ...
      startServer: () => set({ isServerRunning: true, connected: true }),
      stopServer: () => set({ isServerRunning: false, connected: false }),
